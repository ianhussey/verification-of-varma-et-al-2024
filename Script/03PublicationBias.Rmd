...
# Set data paths
dataPath = file.path("../Results/OutlierInfluential/data") # Set data path
date = 20260119
dataPath = paste0(dataPath,"/",date,"/")
dir.create(file.path("../Results/OutlierInfluential/ExpLevel_data/"), showWarnings = FALSE)
dataPath_expLevel = paste0("../Results/OutlierInfluential/ExpLevel_data/",date,"/")

# Set output paths
outputPath = file.path("../Results/PublicationBias")
outputPath_expLevel = file.path("../Results/OutlierInfluential/ExpLevel_data/")
outputPath_expLevel = paste0(outputPath_expLevel,"/",date,"/")
...
dir.create(file.path(outputPath_expLevel), showWarnings = FALSE)
...
exp_calculate <-function(dataName,dataPath,outputPath,outlier = F){
  ...
  write.csv(temp_cal,paste0(outputPath,"ExpLevel_",substring(dataName,30,nchar(dataName)-4),name,".csv"),
            row.names = F)
}
...
Trim_Fill <- function(dataName_expLevel,dataPath_expLevel,outputPath){ 
  ...
  dir.create(paste0(outputPath,"/TrimFill/plots/",date),showWarnings = F)
  png(paste0(outputPath,"/TrimFill/plots/",date,"/FunnelPlot_",substring(dataName_expLevel,10,nchar(dataName_expLevel)-4),".png",sep=""), width = 4, height = 5, units = 'in', res = 300)
...
write.csv(egger,paste(outputPath,"/EggersTest/Results_EggersTest_",date, ".csv",sep=""),row.names = F)
...
write.csv(trimfill_df,paste(outputPath,"/TrimFill/Results_TrimFill_",date, ".csv",sep=""),row.names = F)
...
write.csv(psm,paste(outputPath,"/ThreePSM/Results_ThreePSM_",date, ".csv",sep=""),row.names = F)
