...
dir.create(file.path("../data/processed"), showWarnings = FALSE) # create folder named Results
dir.create(file.path("../data/processed/outlier_influential"), showWarnings = FALSE)
dir.create(file.path("../data/processed/outlier_influential/data"), showWarnings = FALSE)
dir.create(file.path("../data/processed/outlier_influential/plots"), showWarnings = FALSE)
...
computeOutliers<- function(file,dataPath,outputPath,ncore){
  require(metafor)
  require(meta)
  # require(ggplot2)
  # require(gridExtra)
  dataOutputPath = file.path(paste0(outputPath,"/outlier_influential/data"))
  plotOutputPath = file.path(paste0(outputPath,"/outlier_influential/plots"))
  dir.create(dataOutputPath, showWarnings = FALSE)
  dir.create(plotOutputPath, showWarnings = FALSE)
  
  # setwd(dataPath)  <-- REMOVED: setwd inside functions breaks path relativity for subsequent runs
  temp = read.csv(file.path(dataPath, file))
  ...
  for (i in outcome) {
    ...
    if (i == "Intrusion frequency") {
      ...
      for (k in suboutcome) {
        ...
        # write.csv(df_Studentized_Outliers,paste0(outputPath,"/outlier_influential/Results_Studentized_Outliers_",i,"_",k,"_", file)
        #           ,row.names = F)
    ...
    } else { # ADDED: ELSE block so this doesn't run for Intrusion frequency
    ...
    # ADDED: Defined variable that was missing before write.csv
    df_Studentized_Outliers <- df[df$outliers == 1, ]
    
    # Export the outliers(Optional)
    write.csv(df_Studentized_Outliers,paste0(outputPath,"/outlier_influential/data/Results_Studentized_Outliers_",i,"_", file)
              ,row.names = F) # FIXED: Missing closing parenthesis in paste0
...
# run for each file
computeOutliers("AllIntrusionEffectSizes.csv",dataPath,outputPath,ncore)
computeOutliers("IncreaseIntrusionEffectSizes.csv",dataPath,outputPath,ncore)
computeOutliers("DecreaseIntrusionEffectSizes.csv",dataPath,outputPath,ncore)
computeOutliers("UnspecifiedIntrusionEffectSizes.csv",dataPath,outputPath,ncore)
