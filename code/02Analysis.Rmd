...
# Pointing to the specific date folder created in the previous step
dataPath = file.path("../data/processed/outlier_influential/data/") 
outputPath <- file.path("../data/processed/Results/") # Outputting results into processed/Results
...
getTable_Mod <- function(analysisName,file,modformula,modMain = 1,modFactor = NULL,focalColumnName,
  ...
  # Read using the full path passed in 'file' argument
  df = read.csv(file)
  
  # Use only the filename for string parsing logic
  ...
  # Extract the name for this analysis using filename_only
  separate_name = unlist(strsplit(filename_only,"_"))
  temp_name = separate_name[grepl("IntrusionEffectSizes",separate_name)] #prediction
  temp_name_Outcome = str_split_i(filename_only,"_",4)
  
  if (grepl("Intrusion frequency_Intrusion diary",filename_only)) {
    suboutcomeName = "Intrusion diary"
  }else if (grepl("Intrusion frequency_Self-report questionnaire",filename_only)) {
    suboutcomeName = "Self-report questionnaire"
  }else if (grepl("Intrusion frequency_Lab-based intrusion monitoring task",filename_only)) {
  ...
  if (is.null(modType) & is.null(modFactor) & modMain == 1){
    ...
    # WRAPPED IN TRY
    metaObject = try(rma.mv(yi=yi_pos, V = vi, data=df, random = formula(modformula),control = optimizer), silent = TRUE)
    
    # SAFETY CHECK: If try failed, return NULL immediately
    if (inherits(metaObject, "try-error")) {
        return(NULL)
    }
  ...
  if (modMain == 1 & is.null(modType) & is.null(modFactor)) {
    ...
    # ONLY RUN if metaObject is valid
    if (!inherits(metaObject, "try-error") && !is.null(metaObject)) {
    ...
    if (change_optimizer) {
      meta_mod <- try(rma.mv(yi=yi_pos, V = vi, data=df, mods = ~factor(modFactor_prep)-1, random = formula(modformula),control = optimizer2), silent = TRUE)
    }else{
      meta_mod = try(rma.mv(yi=yi_pos, V = vi, data=df, mods = ~factor(modFactor_prep)-1, random = formula(modformula),control = optimizer), silent = TRUE)
    }
...
dir.create(paste0(outputPath,"MainAnalysis/"), showWarnings = FALSE) # Fixed dir creation
MainOutputPath = paste0(outputPath,"MainAnalysis/")
dir.create(MainOutputPath, showWarnings = FALSE)


meta_all = data.frame()
for (i in runlist) {
  # FIX: Skip the "Results_" files so we only analyze actual data
  if (!grepl("^Data_", i)) next
  
  temp = getTable_Mod(file = file.path(dataPath, i), analysisName = "Main Analysis", 
                      modMain = modMain,  modformula= modformula,
                      outlier = F)
  temp_out = getTable_Mod(file = file.path(dataPath, i), analysisName = "Main Analysis", 
                          modMain = modMain,  modformula= modformula,
                          outlier = T)
  meta_all = rbind(meta_all,temp,temp_out)
}

write.csv(meta_all, paste0(MainOutputPath,"/Results_Multilevel_Meta-Analysis.csv"),row.names = F)
...
dir.create(paste0(outputPath,"ModerationAnalysis/"),showWarnings = F)
ModOutputPath = paste0(outputPath,"ModerationAnalysis/NoSubset/")
...
for (i in runlist) {
  # Fix: Only process the "Data_..." files, skip the "Results_..." files
  if (!grepl("^Data_", i)) next
  
  all =  data.frame()
  
  # load data to later check if the moderation level is larger than 2
  check = read.csv(file.path(dataPath, i))
  ...
  for (k in 1:length(moderation_category_runlist)) {
    ...
    tryCatch({temp = getTable_Mod(file.path(dataPath, i),analysisName = paste0("Moderation Analysis: ",moderation_category_runlist[k]), 
    ...
    tryCatch({temp_out = getTable_Mod(file.path(dataPath, i),analysisName = paste0("Moderation Analysis: ",moderation_category_runlist[k]), 
                                      modMain = modMain, modType = cat_mod, modformula= modformula,
  ...
  for (j in 1:length(moderation_continuous_runlist)) {
    tryCatch({temp_con = getTable_Mod(file.path(dataPath, i),analysisName = paste0("Moderation Analysis: ",moderation_continuous_runlist[j]), 
    ...
    tryCatch({temp_con_out = getTable_Mod(file.path(dataPath, i),analysisName = paste0("Moderation Analysis: ",moderation_continuous_runlist[j]), 
                                          modMain = modMain, modType = con_mod, modformula= modformula,
  ...
  write.csv(all, paste0(ModOutputPath,substring(i,1,nchar(i)-4),"_ModerationAnalysis.csv"),
            row.names = F)
...
# Use file.path for robust cross-platform path construction
ModOutputPath = file.path(outputPath, "ModerationAnalysis")
ModOutputPath_SubsetTime = file.path(ModOutputPath, "SubsetTime")
dir.create(ModOutputPath_SubsetTime, showWarnings = F, recursive = TRUE)

# Create the data path to store the data subsets
TimeDataPath = file.path(outputPath, "Subset_Time")
Step3DataPath = file.path(outputPath, "Subset_Step3")
Step4DataPath = file.path(outputPath, "Subset_Step4")

dir.create(TimeDataPath, showWarnings = F, recursive = TRUE)
dir.create(Step3DataPath, showWarnings = F, recursive = TRUE)
dir.create(Step4DataPath, showWarnings = F, recursive = TRUE)

# subset data into different timepoints, different step 3 and step 4 categories
for (i in runlist) {
  
  # FIX: Skip non-Data files to avoid processing previous results
  if (!grepl("^Data_", i)) next 
  
  # Read file
  tempDF = read.csv(file.path(dataPath, i))
  
  # Loop 1: TimeOfManipulation
  for (k in unique(tempDF$TimeOfManipulation)) {
    # Skip if NA
    if (is.na(k)) next 
    
    temp = subset(tempDF, tempDF$TimeOfManipulation==k)
    write.csv(temp, file.path(TimeDataPath, paste0(substring(i,1,nchar(i)-4),"_",k,".csv")),
              row.names = F)
  }
  ...
  for (p in unique(tempDF$Step3_Technique_procedure)) {
    ...
    write.csv(temp, file.path(Step3DataPath, paste0(substring(i,1,nchar(i)-4),"_",p,".csv")),
              row.names = F)
  }
  ...
  for (q in unique(tempDF$Step4_Hypothesized_mechanism)) {
    ...
    if (is.na(q)) {
      write.csv(temp, file.path(Step4DataPath, paste0(substring(i,1,nchar(i)-4),"_Pharmac.csv")),
                row.names = F)
    } else {
      write.csv(temp, file.path(Step4DataPath, paste0(substring(i,1,nchar(i)-4),"_",q,".csv")),
                row.names = F)
    }
  }
...
for (i in runlist_subsetTime) {
  ...
  # FIX: Read from TimeDataPath, not dataPath, because 'i' is from runlist_subsetTime
  check = read.csv(file.path(TimeDataPath, i))
  check = subset(check,!is.na(check$yi_pos))
  ...
  for (k in 1:length(moderation_category_runlist)) {
    ...
    # FIX: Use file.path(TimeDataPath, i)
    tryCatch({temp = getTable_Mod(file.path(TimeDataPath, i),analysisName = paste0("Moderation Analysis: ",moderation_category_runlist[k]), 
                                  modMain = modMain, modType = cat_mod, modformula= modformula,
                                  modFactor = moderation_category_runlist[k],outlier = F)
    # cat(paste0(i,"\n", "Moderator:",moderation_category_runlist[k],"\nOutlier : True"))
    }, 
    ...
    # FIX: Use file.path(TimeDataPath, i)
    tryCatch({temp_out = getTable_Mod(file.path(TimeDataPath, i),analysisName = paste0("Moderation Analysis: ",moderation_category_runlist[k]), 
                                      modMain = modMain, modType = cat_mod, modformula= modformula,
                                      modFactor = moderation_category_runlist[k],outlier = T)
    # cat(paste0(i,"\n", "Moderator:",moderation_category_runlist[k],"\nOutlier : False"))
    },
  ...
  # write.csv(all, paste0(ModOutputPath,substring(i,30,nchar(i)-4),"_ModerationAnalysis_SubSetTime.csv"),
  #           row.names = F)
  # save all outcome as one big file
  mod_all = rbind(mod_all,all)
}

write.csv(mod_all, paste0(ModOutputPath_SubsetTime,"ModerationAnalysis_SubSetTime.csv"),
          row.names = F)
...
ModOutputPath = file.path(outputPath, "ModerationAnalysis")
ModOutputPath_SubsetStep3 = file.path(ModOutputPath, "SubsetStep3")
dir.create(ModOutputPath_SubsetStep3, showWarnings = F, recursive = TRUE)
...
for (i in runlist_subsetStep3) {
  ...
  # FIX: Read from Step3DataPath, not dataPath
  check = read.csv(file.path(Step3DataPath, i))
  check = subset(check,!is.na(check$yi_pos))
  ...
  for (k in 1:length(moderation_category_runlist)) {
    ...
    # FIX: Use file.path(Step3DataPath, i)
    tryCatch({temp = getTable_Mod(file.path(Step3DataPath, i),analysisName = paste0("Moderation Analysis: ",moderation_category_runlist[k]), 
                                  modMain = modMain, modType = cat_mod, modformula= modformula,
                                  modFactor = moderation_category_runlist[k],outlier = F)
    # cat(paste0(i,"\n", "Moderator:",moderation_category_runlist[k],"\nOutlier : True"))
    }, 
    ...
    # FIX: Use file.path(Step3DataPath, i)
    tryCatch({temp_out = getTable_Mod(file.path(Step3DataPath, i),analysisName = paste0("Moderation Analysis: ",moderation_category_runlist[k]), 
                                      modMain = modMain, modType = cat_mod, modformula= modformula,
                                      modFactor = moderation_category_runlist[k],outlier = T)
    # cat(paste0(i,"\n", "Moderator:",moderation_category_runlist[k],"\nOutlier : False"))
    },
...
write.csv(mod_all2, paste0(ModOutputPath_SubsetStep3,"ModerationAnalysis_SubSetStep3.csv"),
          row.names = F)
...
ModOutputPath = file.path(outputPath, "ModerationAnalysis")
ModOutputPath_SubsetStep4 = file.path(ModOutputPath, "SubsetStep4")
dir.create(ModOutputPath_SubsetStep4, showWarnings = F, recursive = TRUE)
...
for (i in runlist_subsetStep4) {
  ...
  # FIX: Read from Step4DataPath, not dataPath
  check = read.csv(file.path(Step4DataPath, i))
  check = subset(check,!is.na(check$yi_pos))
  ...
  for (k in 1:length(moderation_category_runlist)) {
    ...
    # FIX: Use file.path(Step4DataPath, i)
    tryCatch({temp = getTable_Mod(file.path(Step4DataPath, i),analysisName = paste0("Moderation Analysis: ",moderation_category_runlist[k]), 
                                  modMain = modMain, modType = cat_mod, modformula= modformula,
                                  modFactor = moderation_category_runlist[k],outlier = F)
    # cat(paste0(i,"\n", "Moderator:",moderation_category_runlist[k],"\nOutlier : True"))
    }, 
    ...
    # FIX: Use file.path(Step4DataPath, i)
    tryCatch({temp_out = getTable_Mod(file.path(Step4DataPath, i),analysisName = paste0("Moderation Analysis: ",moderation_category_runlist[k]), 
                                      modMain = modMain, modType = cat_mod, modformula= modformula,
                                      modFactor = moderation_category_runlist[k],outlier = T)
    # cat(paste0(i,"\n", "Moderator:",moderation_category_runlist[k],"\nOutlier : False"))
    },
...
write.csv(mod_all3, paste0(ModOutputPath_SubsetStep4,"ModerationAnalysis_SubSetStep4.csv"),
          row.names = F)
...
# Use file.path for robust paths
TechniqueOutputPath = file.path(outputPath, "IndividualTechniqueAnalysis")
dir.create(TechniqueOutputPath, showWarnings = F, recursive = TRUE)
...
for (g in focal_columns) {
  for (i in runlist_subsetTime) {
    ...
    # Optional safety: Skip non-data files if any exist
    if (!grepl(".csv$", i)) next
    
    # FIX: Read from TimeDataPath, NOT dataPath
    test = read.csv(file.path(TimeDataPath, i)) 
    ...
    if (!is.null(dim(test)[1]) & dim(test)[1] >=1) {
      ...
      # FIX: Pass full path to getTable_Mod
      temp = getTable_Mod(file = file.path(TimeDataPath, i), analysisName = paste0("FocalAnalysis_",g), 
                          modMain = modMain,  modformula= modformula, focalColumnName = g,
                          outlier = F)
      
      if (!is.null(temp) && dim(temp)[1] > 0) {
        temp$TimeofManipulation = substr(tail(unlist(str_split(i,"_")),1),1,nchar(tail(unlist(str_split(i,"_")),1))-4)
        # Verify columns exist before subsetting by index to be safe, but keeping minimal changes:
        if(ncol(temp) >= 20) temp = temp[c(1,2,20,3:19)]  
      }
      
      
      # remove outlier
      # FIX: Pass full path to getTable_Mod
      temp2 = getTable_Mod(file = file.path(TimeDataPath, i), analysisName = paste0("FocalAnalysis_",g), 
                           modMain = modMain,  modformula= modformula, focalColumnName = g,
                           outlier = T)
      
      if (!is.null(temp2) && dim(temp2)[1] > 0) {
        temp2$TimeofManipulation = substr(tail(unlist(str_split(i,"_")),1),1,nchar(tail(unlist(str_split(i,"_")),1))-4)
        if(ncol(temp2) >= 20) temp2 = temp2[c(1,2,20,3:19)]
      }
...
write.csv(focal_all, file.path(TechniqueOutputPath, "SubsetTime_IndividualTechnique_Analysis.csv"),
          row.names = F)
